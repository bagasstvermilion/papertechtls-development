...
class Cs extends CI_Controller
{
	...
	public function api_cs_export()
	{
		...
		if (!$dari || !$sampai) {
			echo "Parameter dari & sampai wajib";
			return;
		}
		if ($dari > $sampai) {
			$temp = $dari;
		...
		if (empty($tanggal_list)) {
			echo "Tidak ada data di rentang ini";
			return;
		}
		...
		foreach ($tanggal_list as $tgl_row) {
			...
			$headers_row4 = [
				...
				'T' => 'TOLERANSI',
				'V' => 'WARNING',
				'W' => 'Nama Supir / No Telp',
				'X' => 'Remark (CS / Security / Warehouse)'
			];
			...
			$sheet->setCellValue('E5', 'type');
			$sheet->setCellValue('F5', 'weight');
			...
			$sheet->setCellValue('R5', 'Min');
			$sheet->setCellValue('S5', 'Max');
			$sheet->mergeCells('T4:U4');
			$sheet->setCellValue('T5', 'Min');
			$sheet->setCellValue('U5', 'Max');
			...
			foreach ($rows_db as $db_row) {
				...
				$rowdata = [
					'A' => $db_row['kedatangan_truck'],
					'B' => $db_row['nama_customer'],
					'C' => $db_row['area'],
					'D' => $db_row['urutan_bongkar'],
					'E' => $db_row['item_desc'],
					'F' => $db_row['weight'],
					'G' => $db_row['color'],
					'H' => $db_row['pattern_nose'],
					'I' => $db_row['qty_box_pallet'],
					'J' => $db_row['qty_pcs'],
					'K' => $db_row['qty_box_pallet_total'],
					'L' => $db_row['status_produk'],
					'M' => $db_row['waktu_muat'],
					'N' => $db_row['status_tracking'],
					'O' => $db_row['berat_total'],
					'P' => $db_row['berat_box'],
					'Q' => $db_row['berat_isi_truck'],
					'R' => $db_row['normal_min'],
					'S' => $db_row['normal_max'],
					'T' => $db_row['toleransi_min'],
					'U' => $db_row['toleransi_max'],
					'V' => $db_row['warning'],
					'W' => $nama_telp,
					'X' => $remark_gabung
				];
