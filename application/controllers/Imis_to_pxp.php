...
if ($rs->num_rows() > 0) {
	...
	/* unloading import case 20250226 tukar posisi antara vaning dan shipping code */
	$sql = "INSERT into pxp_ldt.unloading_import_case(plan_date, shippingcode, cont_no, caseno, status, unloading_at, unloading_by, unloading_casemark, vanning_code, upload_at, upload_by, receive_at, receive_by, caseid, containerid, impcode, receive_location) 
			select '1999-09-09' as plan_date, shippingcode, containerno as cont_no, caseno, status, unloading_at, unloading_by, code_qr_casemark as unloading_casemark, vanningcode as vanning_code, upload_at, upload_by, receive_at, receive_by, caseid, containerid, impcode, idlocation from import_case ic 
			where concat(containerno, shippingcode, caseno) in (
	...
	/* shipping plan 20250226 tukar posisi antara vaning dan shipping code */
	insert into pxp_ldt.machining_shipping_plan(id, caseno, partno, rubetsu, partname, qty, containerno, fta_code, vanningcode, shippingcode, efa_mfn, status, created_at, created_by, caseid, containerid, impcode) 
	select 
		ROW_NUMBER () OVER (ORDER by partid) + (select max(id) from pxp_ldt.machining_shipping_plan) as id,
		a.caseno, a.partno, '00' rubetsu, a.partname, a.qty, containerno, '-' fta_code, vanningcode, a.shippingcode, '-' efa_mfn, /*'waiting'*/ status, upload_at created_at, upload_by created_by, caseid, containerid, impcode
	from import_part a
	where concat(containerno, shippingcode, caseno) in (
		select concat(containerno, shippingcode, caseno) from import_case uic where to_char(upload_at,'YYYY-MM') ='$cm' and LEFT(caseno,2) IN ('H1','H2','TY') and status in ('shipping','receive','unloading')  
		and concat(containerno, shippingcode, caseno) not in (
