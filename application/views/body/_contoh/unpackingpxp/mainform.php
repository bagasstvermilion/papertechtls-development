              ...
              <div class="col-lg-4">
                <div class="row">
                  <div class="col-lg-12">
                    ...
                    <div class="input-group">
                      <input id="statusInfo" name="statusInfo" placeholder="Status Information" type="text" class="form-control form-control-lg" readonly style="background-color: ; color:black; text-align:center; font-size: 60px; height:80px;">
                    </div>
                  </div>
                </div>
                <div class="row">
...
<script>
    ...
    $.get(url)
      .done((response) => {
        ...
        if (parseInt(datajson.sts) == 1) { //caseNo sudah complete found
          ...
          get_actualboxbycaseno($('#scmCaseNo').val());
          get_planboxbycaseno($('#scmCaseNo').val());
          //eof 20220921
  ...
  function fun_scanLabelMark() {
    ...
    if (parseInt(datajson1.qty_actual_casenopartno) == parseInt(datajson1.qty_plan_casenopartno)) {
      ...
      play_audio_error();
      $('#statusInfo').val("Complete Part scan other part !");
      $('#statusInfo').css({
        "background-color": "orange",
        "color": "black"
      });
      $('#statusPart').val("Part Complete");
    ...
    if ((parseInt($('#actualBox').val()) + parseInt(slmQty)) > parseInt($('#planBox').val())) {
      $('#statusInfo').val("Qty Actual melebihi Plan");
      document.getElementById('scanLabelMark').focus();
      play_audio_error();
      $('#statusInfo').css({
        "background-color": "red",
    ...
    if (slmCaseNo == $('#scmCaseNo').val()) {
      ...
      $.get(url, dataString)
        .done((response) => {
          ...
          if (datajson.state == "aktual_melebihi_plan" && datajson.sts == 9) {

            play_audio_error();
            $("#statusInfo").val("Qty Part melebihi Plan");
            $('#statusInfo').css({
              "background-color": "red",
              "color": "white"
            });
            ...
            /*  } else if (datajson.state == "duplikat") {
